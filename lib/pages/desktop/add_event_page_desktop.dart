...
class AddEventPageDesktop extends StatefulWidget {
  ...
  @override
  State<AddEventPageDesktop> createState() => _AddEventPageDesktop();
}
  ...
  if (user != null) {
    ...
    if (isBlacklisted) {
      if (mounted) {
        ...
        Navigator.of(context).pop(); // iska saar AddEventPageDesktop
      }
    }
  }
}
...
void _initializeDatesAndTimes() {
  ...
  // 🕓 Haddii shifts la doortay
  if (widget.selectedShifts.isNotEmpty) {
    final allMorning = widget.selectedShifts.every((s) => s.contains("08:00"));
    final allAfternoon = widget.selectedShifts.every((s) => s.contains("02:00"));
    ...
    if (allMorning) {
      ...
      // 🟡 Haddii mix ah, qaado ugu horreeya iyo ugu dambeeya
      final sorted = widget.selectedShifts.toList()..sort();
      if (sorted.isNotEmpty) {
        if (sorted.first.contains("08:00")) firstShiftStr = '8:00 AM';
        if (sorted.last.contains("05:00")) lastShiftStr = '5:00 PM';
      }
    }
  }

  // 🕘 Haddii timeSlot la doortay halkii shift
  ...
  // 🕑 Haddii shift keliya la doortay (e.g. "AM" ama "PM")
  ...
  // ✅ Hubi in la nadiifiyo dhammaan waqtiga kahor inta la gelinayo controller-ka
  _startDateController.text = DateFormat('yyyy-MM-dd').format(start);
  _endDateController.text = DateFormat('yyyy-MM-dd').format(end);
  _startTimeController.text = firstShiftStr.trim();
  _endTimeController.text = lastShiftStr.trim();

  debugPrint('✅ Initialized StartTime: ${_startTimeController.text}, EndTime: ${_endTimeController.text}');
}
...
Future<String?> _uploadBusinessDocument() async {
  ...
  if (await blacklistedDoc(_businessDocName!)) {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('This document is blacklisted. Upload denied.')),
    );
    return null;
  }
  ...
  try {
    ...
    final request = http.MultipartRequest('POST', uri)
      ...
      ..files.add(await http.MultipartFile.fromPath('file', _businessDocument!.path));
...
Future<void> _pickAndUploadImage() async {
  try {
    ...
    if (kIsWeb) {
      ...
      // 📱 Mobile / 🖥️ Desktop upload using file path
...
String _monthName(int month) {
  ...
  builder: (context, snapshot) {
    ...
    return DropdownButtonFormField<String>(
  decoration: const InputDecoration(labelText: 'Category'),
  initialValue: _selectedCategory, // ✅ beddel 'value:' -> 'initialValue:'
  items: categories
