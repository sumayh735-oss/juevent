...
Future<void> _fetchReports() async {
  try {
    ...
    for (var doc in snapshot.docs) {
      ...
      allData.add({
        ...
        'companyLocation': data['companyLocation'] ?? '', // ✅ NEW
    ...
    // ✅ Calculate most active company and its location
    final Map<String, int> companyCount = {};
    final Map<String, String> companyLocations = {};
    for (var r in allData) {
      final company = r['companyName'] ?? '';
      final location = r['companyLocation'] ?? '';
      if (company.isNotEmpty) {
        companyCount[company] = (companyCount[company] ?? 0) + 1;
        // Save the location (if multiple events, the first non-empty will stay)
        if (!companyLocations.containsKey(company) && location.isNotEmpty) {
    ...
    int mostCount = 0;
    String mostCompanyLocation = 'N/A';
    if (companyCount.isNotEmpty) {
      final sorted = companyCount.entries.toList()
        ..sort((a, b) => b.value.compareTo(a.value));
      mostCompany = sorted.first.key;
      mostCount = sorted.first.value;
      mostCompanyLocation = companyLocations[mostCompany] ?? 'N/A';
    }
    ...
    setState(() {
      ...
      _mostcompanyLocation = mostCompanyLocation; // ✅ store location
    });
  } catch (e) {
  ...
  Future<void> _exportExcel() async {
    ...
    sheet.getRangeByName('B2').setText("🏆 Most Active Company: $_mostCompany ($_mostCount events)");
    ...
    for (var i = 0; i < _reports.length; i++) {
      ...
      sheet.getRangeByIndex(i + 4, 10).setText(r['startDateTime'] != null ? DateFormat.yMMMd().format(r['startDateTime']) : "N/A");
